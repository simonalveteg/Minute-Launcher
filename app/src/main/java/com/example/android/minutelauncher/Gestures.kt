...
fun gestureHandler(
  ...
  if (abs(dragAmount.x) > abs(dragAmount.y)) {
    if (abs(dragAmount.x) > threshold) {
      if (dragAmount.x > 0) {
        gestureDirection =
          if (gestureZone == GestureZone.UPPER) GestureDirection.UPPER_RIGHT else GestureDirection.LOWER_RIGHT
      } else if (dragAmount.x < 0) {
        gestureDirection =
          if (gestureZone == GestureZone.UPPER) GestureDirection.UPPER_LEFT else GestureDirection.LOWER_LEFT
      }
    }
